...
- **mcap CSV 期間（BQ欠損の穴埋め）**: BigQuery にデータが上がっていない運行を、
  [GetMcapToCsv](https://github.com/Shintaro-Hirata/GetMcapToCsv) で mcap から抽出した
  トピック別 CSV で補える。サイドバーの「mcap CSV 期間」に CSV をアップロードし、
  対象（Q1/Q2/自由フィールド）と値列を指定して「実行」すると、BQ 由来の期間と
  同列に散布図・比較・表・ヒストグラムで統計比較できる。
  `t2_system_state_manager_state` の CSV も一緒に入れると BQ と同じ
  「自動運転（system_state=4）のみ」の絞り込みが適用される。
  緯度経度・移動距離は CSV に無いため、横軸は時刻／経過時間を使用（地図は非対応）
- **表示レンジ外データの警告**: 散布図・ヒストグラムの表示レンジを設定したとき、
  その範囲の外にデータがある（＝グラフから隠れている）場合は該当グラフに警告を表示。
  さらに、レンジを変えた瞬間に新たにレンジ外が生じると、画面右下にトースト通知
  （Windows風のポップアップ）でも知らせる。設定JSONを読み込んでレンジが変わった
  場合も同様に検知する（タブ切替などレンジに無関係な操作では通知しない）
- **散布図⇔地図⇔表**: 各グラフは切替ボタンで地図表示（OSM）・表表示に変更可能。
  地図の色は「期間ごとの色」／「値の大きさグラデーション」を選択でき、
  期間色はカラーピッカーで変更可能。グラデーション表示で期間が複数あるときは
  期間ごとに地図を分けて描画し、どの期間のどこで値が大きいかを判別できる。
  グラデーションの色スケール（|値|）の下限・上限は指標／自由フィールドごとに
  固定でき、期間を分けても色の意味が揃う
...
- **自由フィールド（任意テーブル×列）**: サイドバーの「自由フィールド」で
  ...
  地図を自動スキップ。ヒストグラムは自動/手動で分割（横軸ラベルはフィールドの
  ラベルを使用）。表示レンジ（散布図・画像・ヒストグラムのX/Y）はフィールドごとに
  独立して変更でき、フィールド定義・表示レンジとも設定JSONに保存・復元できる。
  複数追加に対応
