...
- **CSV 形式**: GetMcapToCsv 出力（`t_ns` 列あり）をそのまま。加えて汎用形式
  （1列目=時間: epoch 秒/ミリ/ナノ or JST 日時文字列、2列目以降=値。値列は複数可）も
  可（`read_value_csv`）。
- **1つの CSV から複数指標を置き換え可**: パネルの「この CSV から置き換える指標の数」を
  増やすと、同じ CSV の別々の列を別々の指標へ同時に適用できる（行ごとに
  対象・値列・scale/offset を指定。`OverrideEntry` が指標ごとに 1 件でき、
  `_apply_rows` は同一 CSV のパースを 1 回にキャッシュする）。レシピも期間×対象ごとに残る。
