...
from src.data_service import (
    _add_ratio,
    _aggregate_hist_bins,
    _concat_make_cum_dist_continuous,
    _is_resource_limit_error,
)
...
def test_concat_cum_dist_continuous_offsets():
    ...
    out = _concat_make_cum_dist_continuous([df_a, df_b])
...
def test_concat_cum_dist_skips_empty_and_handles_missing_col():
    ...
    out = _concat_make_cum_dist_continuous([df_a, df_none, df_nocol])
    assert len(out) == 2
    assert "x" in out.columns


def test_concat_cum_dist_all_empty_returns_empty():
    out = _concat_make_cum_dist_continuous([pd.DataFrame(), None])
    assert out.empty
...
def test_aggregate_hist_bins_sums_counts():
    ...
    out = _aggregate_hist_bins([df_a, df_b])
...
def test_aggregate_hist_bins_ignores_invalid():
    ...
    out = _aggregate_hist_bins([bad, pd.DataFrame(), None])
    assert out.empty


def test_add_ratio_sums_to_one():
    df = pd.DataFrame({"bin_start": [0.0, 0.2], "cnt": [1, 3]})
    out = _add_ratio(df, cnt_col="cnt", ratio_col="ratio")
